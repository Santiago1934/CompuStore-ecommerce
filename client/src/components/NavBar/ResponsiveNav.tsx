  ...
  return (
    <div className={styles.header}>
      <div className={styles.containerSearchLogo}>
        <Link to='/' > <h1 className={styles.LogoCH} >CompuStore</h1> </Link>
      ...
      <ul className={click ? styles.nav_options_active : styles.nav_options}>
        ...
        <div className={styles.optionLogin} >
          {user?.name ? false :
            <Link to="/login">
              <button className={styles.buttonLogin} > {/* <FontAwesomeIcon icon={faUserAlt} style={{ marginRight: '5%' }} /> */}Login</button>
            </Link>}
          <p>{user?.name &&
            <NavBarItem info="Cuenta"></NavBarItem>}</p>
        </div>
